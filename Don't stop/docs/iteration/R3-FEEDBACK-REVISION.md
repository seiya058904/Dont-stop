...
- 第 5 项：浏览器里返回后**间歇性**无法立刻再开始（见上）；阻断开火对照的判据余量偏薄。
  后续：本项已在 [R4 A批](R4-FEEDBACK-REVISION.md) 修到根因（返回时残留已释放武器节点），5 轮原生（128/0）与 5 轮浏览器验收均通过，工作流里的 `KNOWN_FAILING_TRACKED` 豁免已删除、改为非零退出阻断门禁；但 R4 改动尚未合并、尚未部署。这里保留 R3 当时的实际状态不改写。阻碍对照的「开火像素探针」在 R4 被确认判据方向相反，已降级为记录项（同样见 R4 报告）。
