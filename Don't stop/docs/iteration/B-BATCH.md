...
| Boss | 构筑 | TTK | 结果 | 说明 |
| --- | --- | --- | --- | --- |
| B01 | full | ~48s | clear | 目标 45–75s ✓ |
| B02 | full | ~61s | clear | 目标 60–90s ✓ |
| B03 | full | ~80s | clear | 目标 75–120s ✓ |
| B04 | B5Bosses | 见 §11 | 见 §11 | 目标 100–160s |

`high`（中型构筑）在本轮常以 90–100% 血量收尾或阵亡：这是三阶段从 70% 就加速、
...
渲染部分（`gl_compatibility`）：同一张 R6 地图、同一玩家位置的 A/B 对照
（`fog-A-normal-stage26` / `fog-B-hell-profile-same-map` / `fog-A2-restored-same-map`）
给出径向亮度分带，远场压暗到正常亮度的 30% 以下、光照半径内保持可读，
恢复后远场回来。截图与亮度数字见 `docs/iteration/evidence/b/`。
...
3. **怪物之间的互相推挤**会让个别单位短暂贴进墙边几何。
    连续 1 Hz 采样实测为 0.1–2.6% 的采样，`unreachable_stuck` 在 driving 模式为 0。
    出生存放合法性由 `R3SpawnAudit` 严格判 0（真实碰撞体查询）。这是物理推挤的既有表现，
    本轮只是把它**测量出来**，没有引入新的卡墙来源。
4. **`M8UI`(24) 与 `M10RewardAudit`(7)、`M5World`(6) 在 `main` 上就是红的**，
    与本轮无关（已在 `d6652cb5` 上复现同样数量），本轮未修也不掩盖。
    `M5World` 的 6 项里 3 项来自 `M7Fixtures.legacy()` 的既有 `attachments` 取值错误。
5. **`R3SpawnAudit` 的 `TIME_BUDGET_MS` 为 7 分钟**，加入 R7/R8 与新 roster 后接近上限；
    超时会打印 `truncated=1` 并判 FAIL，不会静默通过。
6. `BossUltimate` 的 `B04` 安全区漂移速度（24 px/s）只做过一次校准，可能需要调整。
