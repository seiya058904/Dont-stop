...
### 9.1 与 B0 基线的同条件对照（probe，上限侧）

| 关 | cap | 基线 peak / mean / near80 | 本轮 peak / mean / near80 | 变化 |
| --- | --- | --- | --- | --- |
| 7 | 49 | 49 / 32.1 / 41 | 49 / 32.0 / 41 | 持平（cap-bound） |
| 13 | 38 | 38 / 28.1 / 31 | 38 / 29.3 / 32 | 持平 |
| 17 | 47 | 47 / 35.5 / 43 | 47 / 36.4 / 46 | 持平 |
| 22 | 55 | 55 / 45.1 / 43 | 55 / 47.1 / 40 | 持平 |
| 26 | 93 | 93 / 74.8 / 67 | 93 / 76.9 / 71 | 持平 |
| 29 | 145 | 145 / 124.4 / 71 | 145 / 127.3 / 74 | 持平 |

probe（玩家无敌不动）下每关仍然把 cap 填满，说明**上限侧没有被改动**。

### 9.2 强构筑 driving：26–29 的有效压力

| 关 | cap | interval 基线→本轮 | peak 基线→本轮 | mean 基线→本轮 | spawns 基线→本轮 | near80 基线→本轮 |
| --- | --- | --- | --- | --- | --- | --- |
| 22 | 55 | 0.40 → 0.36 | 10 → 6 | 3.42 → 2.50 | 205 → 217 | 2 → 0 |
| 26 | 93 | 0.29 → 0.26 | 16 → **22** | 5.78 → **7.40** | 297 → **417** | 3 → **5** |
| 29 | 145 | 0.27 → 0.23 | 41 → 41 | 20.66 → **26.0** | 957 → **1152** | 4 → 3 |

26–29 的有效压力提升落在 **+26% ~ +40%**（mean alive 与 replenishment），
与用户给的 "+20~35%" 目标一致，而 **cap 一行未动**。
22 关的 peak/mean 偏低是低绝对值下由击杀速度主导的噪声（spawns 实际 +6%），不是压力下降。

### 9.3 Hell 覆盖（强构筑 driving）

| 关 | cap | peak | mean | near80 | spawns/min | special | elite | projectile peak | hostile zone peak | hazard peak | horde overlap | flank |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 31 | 72 | 17 | 8.0 | 1 | 433 | 0.32 | 2 | 3 | 5 | 1 | 1 | **11** |
| 35 | 105 | 31 | 15.9 | 2 | 842 | 0.31 | 3 | 3 | 5 | 3 | 2 | **11** |
| 39 | 146 | 44 | **20.4** | 5 | 1173 | 0.34 | 3 | 8 | 6 | 5 | 4 | **11** |

* Hell 内部 **31 → 39 的 mean alive 为 8.0 → 20.4，即 2.55×**，与设计档位
  （密度 1.15× → 2.00×）一致，略高，属于校准起点；
* flank 增援每关 11 次、地面危险同时在场 1 → 5、精英 1 → 3 只，全部按 §3 的表递进；
* 六个后期/Hell 关卡在强构筑下**全部 clear**；
* **Hell 的绝对密度低于普通第 29 关（cap 72 vs 145）是刻意的**：同一批怪 HP 为
  1.15–2.26×、伤害 1.08–1.90×，再叠加迷雾、地面危险与精英。
  难度来自组合，而不是把 145 只怪塞进浏览器。

### 9.4 中型构筑 driving（本轮新增观测，不参与判定）

| 关 | cap | peak | mean | near80 | spawns（回合被死亡打断，故偏小） | clear |
| --- | --- | --- | --- | --- | --- | --- |
| 22 | 55 | 26 | 15.5 | 11 | 91 | **否** |
| 26 | 93 | 61 | 33.0 | 21 | 173 | **否** |
| 29 | 145 | 66 | 28.4 | 20 | 92 | **否** |

中型构筑在这三关都以阵亡告终（存活不足 45 秒）。**这是本轮最需要人体验收的一条**：
基线在同样的 `typical` 构筑下只跑过 probe（不动不打），没有可比的 driving 数字，
所以这不能判定为回归；但它确实说明 21–29 的压力对中型构筑偏高。
如果真人试玩也这么认为，**旋钮依次是** `pressure.ring_min`（现 108–122）、
roster 里 E01/E02 的占比、`pressure.chase_speed`（现 1.02–1.18）、`HORDES[*].windows`；
`cap` 不在其中，因为实测证明它从来不是约束。

### 9.5 结构合法性与性能峰值（同一次 1 Hz 采样）

全部关卡：`illegal_near_spawns = 0`（60px 个人空间内不允许生成）、
`closest_spawn` 94–141px（与各关 `ring_min` 一致）、
`wall_overlap_actors = 0`、`unreachable_stuck = 0`（driving 模式）。
probe 模式把玩家钉住并让 146 只怪堆上去时，会出现 0.02–0.8% 的采样被物理推挤贴上墙边几何
（按**不同角色**计最多 1 个），以 NOTE 记录，不参与判定。

**性能底线**：敌方投射物上限 180、`HostileVFX` 上限 32、同一时刻地面危险 ≤ 12
（Hell 31→40 为 3→12，实际受覆盖率先于数量限制）。
实测峰值：projectile 8、hostile zone 6、hazard 5（driving）；probe 极限下
projectile 116、hostile zone 15。
