...
- Run `35605669846` validates the driver correction: cold first-response times are 11.834/10.381/10.452 s instead of roughly 25-26 s; actual hover feedback remains 1.3-1.7 s, so startup still fails. The correction removes measurement overhead rather than improving rendering.
- CI IndexedDB observations show completed configuration/shader transactions and no aborts, but no camp file put at all, including a later completed synchronization. A read-only probe now reports the mounted browser FS file names/stat metadata and syncing flag to localize this discrepancy. Local runtime verification sees the real camp file under `/userfs` with no page errors (`fs-bridge-local.json`); L1 passes in 24.391 s (`l1-20260921-213545/`).
- Single-core SwiftShader plus CPU-throttling still preserves the save, but reproduces a test false failure: an old `[e2e] state` message arrives while navigation is pending and contaminates the new page's probe-only assertion (`save-sync-onecore/`). Console state now resets at main-document navigation, not before `goto`. A deliberate one-second pre-navigation delay regression passes all 53 checks in 58.846 s (`menu-document-boundary/`).
- A rejected 15-second P experiment attempted to retain ordinary enemies with an HP fixture: peaks reached 180 enemies and 180 shots but adjacent-sample simultaneous coverage remained zero; on-screen peaks were only 103/121. The experiment was reverted and is not accepted as a workload correction (`pressure-hold-diagnostic/`). The mismatched-artifact startup fault still exits 1 and retains a FAIL report (`startup-driver-hash-fault/`).

### Browser persistence delay: candidate fix

Run `35606886652` passes the old readability-only menu check, but the transaction evidence contains a camp write only in the fixture document. The later in-memory file is 530 bytes while the earlier saved fixture is 526 bytes. Merely reading a parseable old snapshot is insufficient. The gate now compares the last in-memory save's SHA-256 against the file read immediately after reload, without flushing storage for the game.

The pinned engine's `IDBFS.getRemoteSet` uses one asynchronous timestamp-index cursor callback per file, matching the [upstream implementation](https://github.com/emscripten-core/emscripten/blob/main/src/lib/libidbfs.js). Under long main-thread frames this stretches enumeration across many frames, delaying synchronization of changes made after its initial local snapshot. A real IndexedDB component experiment with 40 files and deliberately busy 35 ms frames measures 1472.9 ms for the original cursor versus 105.2 ms for two bulk reads; keys and duplicate timestamps match exactly. This is a controlled diagnostic, not a game-performance claim.

Web initialization now replaces only remote-index enumeration with two reads of the same index in one read-only transaction. Normal IDBFS reconciliation, file writes and completion/error handling remain in charge. Browsers without bulk APIs retain the original implementation. Bulk reads also transfer the stored values temporarily; this trades a profile-data copy for avoiding per-file main-thread callbacks. Empty database, duplicate timestamps, closed-database errors, transaction abort, idempotence and API fallback are covered by the browser contract.

The actual local game passes all 54 strengthened menu checks in 59.491 s (`idb-batch-menu/`), including identical latest-save hashes before/after reload. L1 passes in 24.594 s (`l1-20260921-214944/`). One startup regression passes cold 6093.585 ms / warm 2237.125 ms, hover 5 ms, Settings 22 ms and Start 68 ms; menu max 33.33 ms, no >50 ms frames (`idb-batch-startup/`).

Run `35608744336`, testing `71ed3cf`, confirms the persistence fix on Linux CI: all 54 menu-return checks pass in 182.803 s. The latest in-memory and reloaded save both hash to `971bf88b19f0c6bd35e317b83b8cf2c0de17ac0e7f2b165cf198d1d7da1a9826`. IndexedDB observations now include four camp-file puts in the actual test document, rather than only the earlier fixture. Evidence: `ci-idb-batch-menu/`. This validates the exercised save/reload sequence, not arbitrary immediate-tab-close durability.

Build, stable preflight and smoke also pass, but startup still fails: cold first-response times are 12.123 / 10.430 / 10.507 s; the baseline menu has seven samples with a 1400 ms maximum, all over 1000 ms (`ci-idb-batch-startup/`). The complete workflow takes about 5 min 41 s but remains failed, so it cannot count as a successful CI timing run. No real GPU CI runner is available, as confirmed by the user; no runner registration or gate relaxation is authorized or performed. Hardware-backed local evidence remains separate from Linux SwiftShader evidence. Startup rendering and formal pressure acceptance remain open.
