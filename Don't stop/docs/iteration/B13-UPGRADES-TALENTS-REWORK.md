...
| # | Finding | Fix |
|---|---------|-----|
| 1 | Growth-evidence provenance pinned only `tests/B13GrowthBench.gd`; editing any measurement input could silently rotate the meaning of the frozen JSON | `docs/iteration/evidence/b13/manifest.json` is now the **measurement-input manifest**: 24 files covering the harness chain (B13GrowthBench + M9Power + M8Runtime + M3Weapons + the bench `.tscn`), build composition & stat path (`DemoConfig`, `AttachmentCatalog`, `EffectiveStats`, `WeaponCatalog`), the runtime combat path (`Demo`, `Combat`, `PlayerData`, `Utils`, `RewardServer`), the benchmark weapon id 0 and its projectile/hit path (`BaseGun`, `GunSprite.gd/.tscn`, `Bullet`, `SmpBullet.gd/.tscn`) and the dummy's HP accounting (`BaseMonster`, `Monster2.gd/.tscn`), plus `game/config/LevelProgression.gd` — `PlayerData` preloads it as PROGRESSION and the bench's per-scenario `player_level = 1` reset runs `PROGRESSION.damage(level)` through the setter into the level-damage term of `EffectiveStats.calculate`, so `DAMAGE_PER_LEVEL` moves the measured DPS. LF-normalised SHA-256, same convention as B12. `B13Strength` verifies: manifest non-empty, exactly the required set of 24 files (no gaps, no extras), every file exists, every hash matches — any drift FAILS until the bench is re-run and the manifest regenerated via `tools/b13_manifest.py` (which parses the required list out of B13Strength.gd, so generator and verifier cannot drift). Deliberately excluded and documented: LevelServer (only its state flag is flipped), Hero (frozen, never hit), save/UI layers. |
...
| 7 | Migration tests only proved refunds at CURRENT B13 prices; nothing pinned a real B12-era ledger | New hardcoded B12-era fixture in B13Economy: T01 rank1 gold **100**, T02 rank1 points **1**, plus a mixed rank (T02 rank2 gold **100**). Product fact from the B12 baseline `6bae6e9`: talent gold pricing was FLAT — `TALENT_GOLD_PRICE = 100` for every talent at every rank (points 1), so 100/100 is the genuine historical amount. The fixture array is hardcoded and never generated from `DemoConfig.talent_gold_price`; sanity checks pin that the B13 prices for these ranks differ (150 / 550 gold, 2 points). Verified against the hardcoded array (not a post-load copy): snapshot validates, load leaves every id/level/currency/amount untouched, `reset_preview()` replays exactly **200 gold / 1 point**, `reset_talents` moves the real wallet by exactly +200 gold / +1 point (NOT the B13 prices), and after save → reload the wallets persist with talents and payments still empty and a 0/0 preview. |
