...
2. **Stage 29 的 horde 基本上打不完**：`alive_mean` 64.5、`alive_peak` 145 = **cap**，
   ...
   不是伤害的问题。
3. **3 次死亡是真的身体围死**（body enclosure）：22/9105、26/9204、28/9105，
   死亡瞬间 49 个探针点里只有 1 个可达、且持续 0.8–0.9 s（denial streak 8–9）。
   这不是 hazard 预警重叠造成的，是身体压力。按 §7-A 属于 knob A（`ring_min`/`chase_speed`/
   `HORDES.windows`），但本轮按 §13/§19 不动。
