...
(async()=>{
 ...
 try{
  ...
  // Filter excludes slot 0; the real click must still open its own detail.
  await click('loadout-select_slot-0');
  await until(()=>state.selection==='0' && state.heading.includes('步枪'),'slot detail independent of filter');
  assert(state.detail_height>=66 && state.detail_scroll===0);
  for(const viewport of [{width:1366,height:768},{width:1536,height:864},{width:1920,height:1080}]){
   await page.setViewportSize(viewport);await sleep(600);
   canvas=await page.locator('#canvas-host canvas').boundingBox();
   assert(state.detail_height>=66);
   await page.screenshot({path:path.join(out,'camp-'+viewport.width+'.png')});
  }
