...
//
// Usage: node web-aim-e2e.js <url> [evidenceDir] [all|core|fault]
//   E2E_HEADED=1        run a real windowed Chromium (required for the OS focus phase)
//   E2E_WATCHDOG_MS=N   whole-run watchdog (default 10 min; see the mode table below)
//
// ---------------------------------------------------------------------------
// WHY THIS FILE TAKES A PHASE SELECTOR
// ---------------------------------------------------------------------------
// Measured on run 35056975522, this gate was 7 m 11 s with the fire chain and the
// aim chain already reading state instead of pixels. The remaining cost is the
// runner's own frame rate (the job log records `fps=1` in the software-rendered
// Web export): every state transition a contract waits on costs at least one
// frame, so the only thing left to buy back was the phases' SERIALISATION. Phase
// F spends 78 s waiting out the loader's own 45 s quiet window and shares not one
// byte of state with A or B, so it runs as its own job alongside them.
//
// This changes WHEN the phases run, never WHETHER they run: both invocations
// assert their own slice in full, the gate needs both jobs, and the union of the
// two required-token sets is the same 65 contracts as before. No assertion was
// deleted, weakened, or moved to a cheaper equivalent.
//
//   all    A + F + B   default: a bare `node web-aim-e2e.js <url> <dir>` behaves
//                      exactly as it always has. Local runs use this.
//   core   A + B       the `aim-core` CI job: real entry (launch handshake, then
//                      aim / crosshair / 360 / WASD / Esc) followed by the fire
//                      chain read from the probe channel.
//   fault  F           the `aim-fault` CI job: fault injection on its own.
//
// Phases, in order:
...
if (!url) {
	console.error('usage: node web-aim-e2e.js <url> [evidenceDir] [all|core|fault]');
	process.exit(2);
}
fs.mkdirSync(outDir, { recursive: true });

// Which of the three phases this invocation runs. See the header: `all` is the
// unchanged default, and the two CI jobs pass `core` and `fault`.
const PHASE_ARG = (process.argv[4] || 'all').toLowerCase();
const PHASES = (() => {
	if (PHASE_ARG === 'all') return new Set(['A', 'F', 'B']);
	if (PHASE_ARG === 'core') return new Set(['A', 'B']);
	if (PHASE_ARG === 'fault') return new Set(['F']);
	// An explicit subset, e.g. "a,b", so one phase can be run alone while
	// debugging without editing this file.
	const set = new Set(PHASE_ARG.split(',')
		.map(s => s.trim().toUpperCase())
		.filter(s => s.length === 1 && 'AFB'.includes(s)));
	if (!set.size) {
		console.error(`usage: node web-aim-e2e.js <url> [evidenceDir] [all|core|fault] (got "${PHASE_ARG}")`);
		process.exit(2);
	}
	return set;
})();
const want = p => PHASES.has(p);

const headed = process.env.E2E_HEADED === '1';
// A budget sized to the phases this invocation actually runs, so a wait is never
// dominated by phases it skipped. If any of these ever needs longer, something is
// wrong and the job has to say so instead of hanging until the workflow's own
// timeout. E2E_WATCHDOG_MS still overrides everything.
const WATCHDOG_BY_MODE = { all: 10 * 60 * 1000, core: 10 * 60 * 1000, fault: 6 * 60 * 1000 };
const WATCHDOG_MS = parseInt(
	process.env.E2E_WATCHDOG_MS || String(WATCHDOG_BY_MODE[PHASE_ARG] || WATCHDOG_BY_MODE.all), 10);
...
(async () => {
	...
	let rect = null;
	// Filled by phase A, read by the evidence file in every mode - so it is
	// declared out here rather than inside the phase A guard.
	let bootLines = [];
	...
	// ---- A: the real entry. Part of the `core` invocation (A+B).
	if (want('A')) {
		...
		bootLines = gameLines.filter(l => l.includes('[boot]'));
	...
	// ---- F: fault injection. Runs as its own invocation, in parallel with
	// `core`, so its 78 s of quiet-window waiting is off the critical path. It is
	// NOT dropped: the contract is asserted in full, and the gate fails if this
	// job fails.
	if (want('F')) {
	...
	// ---- B: the fire chain, read from state. Part of the `core` invocation.
	if (want('B')) {
	...
	// Labelled `errors`, not `B-errors`: this block runs in every mode, so the
	// fault-only job reports its own error verdict under the same name.
	timing('errors');
	...
	// The gate's contracts, grouped by the phase that proves them. A skipped phase
	// contributes no tokens, so the requirement list has to be built from the
	// phases this invocation actually ran - otherwise a `fault` run would "fail"
	// 61 tokens it deliberately never measured. The grouping is what makes the
	// split safe: nothing that runs is allowed to require less than the group it
	// belongs to, and the union across the two CI jobs is still every contract
	// this gate has always carried (41 + 4 + 14 + 6 = 65).
	const REQ_A = [
	...
	// Shared, and asserted by EVERY invocation over whatever phases it ran: the
	// error listeners stay attached across all three loads, so the fault job now
	// carries its own independent "no engine/page/network error, no pointer lock,
	// no renderer crash, within budget" verdict instead of inheriting one from a
	// job that ran different phases. That is strictly more checking than before,
	// not less.
	const REQ_SHARED = [
		'NO_UNEXPECTED_ENGINE_ERRORS', 'NO_PAGE_ERRORS', 'NO_NETWORK_ERRORS',
		'NO_POINTER_LOCK_EVER', 'NO_RENDERER_CRASH', 'WITHIN_THE_RUN_BUDGET',
	];
	const FULL_CONTRACT = REQ_A.length + REQ_F.length + REQ_B.length + REQ_SHARED.length;
	...
	fs.writeFileSync(path.join(outDir, 'web-aim-e2e.json'), JSON.stringify({
		...
		// Which invocation produced this evidence, and what the whole gate covers
		// when both CI jobs are taken together.
		mode: PHASE_ARG, phases_run: [...PHASES].sort(), contracts_across_both_jobs: FULL_CONTRACT,
	...
	if (headed && want('A')) required.push('FOCUS_LOSS_PAUSES', 'REGAIN_FOCUS_DOES_NOT_SELF_FIRE', 'RESUME_AFTER_FOCUS_LOSS');
	...
	console.log(`[aim-e2e] token counts: ${required.length} required, ${failed.length} false, ${Object.keys(tokens).length} total ` +
		`(mode=${PHASE_ARG}: phases ${[...PHASES].sort().join('')}; the gate's full contract set is ${FULL_CONTRACT} tokens, split across the two CI jobs)`);
	if (failed.length) {
