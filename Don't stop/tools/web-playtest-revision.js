...
(async()=>{
 ...
 async function click(tag){await until(()=>rects[tag]?.visible,'missing '+tag);const r=rects[tag];await page.mouse.click(canvas.x+r.x/410*canvas.width,canvas.y+r.y/230*canvas.height,{delay:100});await sleep(400);}
 ...
 try{
  ...
  await click('camp-stage-tab');await find('stage-39');await click('camp-depart-button');await until(()=>combat?.mode==='COMBAT'&&combat.stage===39,'real39');
  ...
  await page.reload();await enter();await click('camp-stage-tab');await find('stage-40');await click('camp-depart-button');
