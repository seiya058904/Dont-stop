...
(async()=>{
 ...
 try{
  ...
  for(const viewport of [{width:1280,height:720},{width:1366,height:768},{width:1536,height:864},{width:1920,height:1080}]){
   ...
   for(const tab of ['weapon','attachment','magazine','stage','talent']){
    await click('camp-'+tab+'-tab');
    await page.screenshot({path:path.join(out,tab+'-window-'+viewport.width+'.png')});
   }
   assert(camp.detail>=66,'details retain readable space');
  }
