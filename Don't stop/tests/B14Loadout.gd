...
func _ready():
	...
	await wait(0.1)
	check(panel.loadout_box.get_child_count() == 2,"camp creates slot heading and row")
	check(panel.loadout_box.get_child(1).get_child_count() == 7,"camp actually renders seven fixed slots")
	panel.queue_free()
	await wait(0.1)
	complete()
