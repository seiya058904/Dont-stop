...
func _ready():
	...
	for stage in stages:
		...
		Demo.kill_stacks=0;Demo.stack_time=0;Demo.refresh();configure(115)
		...
		while LevelServer.state=="COMBAT" and not Utils.player.is_dead and Time.get_ticks_msec()-start<47000:
