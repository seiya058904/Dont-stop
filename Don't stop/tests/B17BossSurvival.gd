...
func _ready():
	...
	for stage in [10,20,30,40]:
		...
		while LevelServer.state=="COMBAT" and not Utils.player.is_dead and Time.get_ticks_msec()-start<120000:
			...
			timeline.append({"s":(Time.get_ticks_msec()-start)/1000.0,"hp":PlayerData.player_hp,"boss_hp":bosses[0].HP if not bosses.is_empty() else 0,"shots":get_tree().get_nodes_in_group("enemy_shot").size(),"position":[Utils.player.global_position.x,Utils.player.global_position.y]})
		stop()
		var row={"stage":stage,"seed":808,"weapon":112,"completed":LevelServer.state=="CAMP" and not Utils.player.is_dead,"dead":Utils.player.is_dead,"hp":PlayerData.player_hp,"movement":movement,"timeline":timeline,"observer":"normal HP autonomous movement and shooting; no HP refill, no human acceptance"}
		output.append(row)
		print("B17_BOSS_SURVIVAL ",JSON.stringify(row))
		var file=FileAccess.open("res://evidence/visual-upgrade-20260919/b17-boss-survival.json",FileAccess.WRITE)
		file.store_string(JSON.stringify(output,"\t")); file.close()
		check(row.completed,"normal-health boss completion "+str(stage))
	print("B17 BOSS SURVIVAL checks=",checks," failures=",failures)
