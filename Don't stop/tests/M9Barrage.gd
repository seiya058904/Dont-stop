...
func _ready():
	...
	for role in ["B01","B02","B03","E10","E11"]:
		for second_phase in [false,true]:
			for index in [0,1,2]:
				...
				rows.append(row); print("M9_PATTERN ",JSON.stringify(row))
				await clean()
				...
				var file = FileAccess.open("res://docs/iteration/evidence/m9/barrage-"+suffix+".json",FileAccess.WRITE); file.store_string(JSON.stringify(rows,"\t")); file.close()
	...
	print("M9_BARRAGE checks=",checks," failures=",failures," incoming_pressure=",pressures)
