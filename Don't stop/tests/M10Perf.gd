...
func _ready():
	...
	for arg in OS.get_cmdline_user_args():
		if arg in ["normal","late","density","boss","projectile","telegraph","particle","boss-barrage"]: scenario = arg
	...
	var counts = {"normal":12,"density":100,"late":70,"boss":0,"projectile":150,"telegraph":24,"particle":30,"boss-barrage":0}
	...
	if scenario=="boss":
		var boss = M5Content.spawn("B03",root,Utils.player.global_position+Vector2(140,0)); boss.HP = boss.max_hp*0.45
	var barrage_owner = null
	if scenario=="boss-barrage":
		barrage_owner = M5Content.spawn("B02",root,Utils.player.global_position+Vector2(140,0)); barrage_owner.HP = barrage_owner.max_hp*0.45
	var frames = []; var peak = 0; var effects_peak = 0; var start = Time.get_ticks_usec(); var previous = start; var clock = 0.0
	var zones_peak = 0; var replenishments = 0; var monsters_peak = 0; var enemy_peak = 0
	while Time.get_ticks_usec()-start<20000000:
		...
		monsters_peak = maxi(monsters_peak,get_tree().get_nodes_in_group("monsters").filter(func(n): return not n.is_die and not n.training).size())
	...
	row.monsters_peak = monsters_peak; row.enemy_projectile_peak = enemy_peak
	...
	var file = FileAccess.open("res://docs/iteration/evidence/m10/perf.json",FileAccess.WRITE); file.store_string(JSON.stringify(row,"\t")); file.close()
	print("M10 PERF ",JSON.stringify(row)); LevelServer.return_to_camp(); dismiss(); await wait(0.8)
