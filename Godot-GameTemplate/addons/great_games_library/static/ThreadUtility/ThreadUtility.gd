...
static func load_resource_list(data_list:Array[Dictionary], finish_callback:Callable = Callable())->void:
	# Scene resources with scripted subresources must be loaded on the main
	# thread. Loading this list from a worker can race global script registration
	# during the boot scene and make actor/player scenes fail to parse.
