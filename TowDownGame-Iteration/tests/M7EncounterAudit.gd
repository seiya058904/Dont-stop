...
func _ready():
	...
	await wait(0.3)
	Demo.save_path = "res://evidence/m7-audit-"+tier+".json"
	get_tree().node_added.connect(observe_spawn)
	Utils.player.incoming_hit.connect(observe_pressure)
	...
	for stage in range(1,31):
		...
		print("M7 AUDIT ROW ",JSON.stringify(row))
		var file = FileAccess.open("res://docs/iteration/evidence/m7/audit-"+tier+".json",FileAccess.WRITE)
		file.store_string(JSON.stringify({"configuration":configuration,"rows":rows,"failures":failures},"\t")); file.close()
	print("M7 AUDIT COMPLETE tier=",tier," rows=",rows.size()," failures=",failures)
...
func observe_pressure(raw: float, applied: float, boss: bool):
	if not observing: return
	row.incoming_raw += raw
	row.incoming_applied += applied
	if boss:
		row.boss_raw += raw
		row.boss_applied += applied
