...
func _ready():
	...
	for stage in [10,20,30]:
		...
		rows.append(row); print("M9 BOSS ",JSON.stringify(row))
		...
		var file = FileAccess.open("res://docs/iteration/evidence/m9/"+prefix+tier+suffix+".json",FileAccess.WRITE); file.store_string(JSON.stringify(rows,"\t")); file.close()
	...
	print("M9 BOSSES SUMMARY checks=",checks," failures=",failures)
