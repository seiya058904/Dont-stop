...
func _ready():
	...
	for gun in PlayerData.player_weapon_list.values():
		pass # Global ownership applies without equip.
		...
		await clean()
		var file = FileAccess.open("res://docs/iteration/evidence/m8/full-build-dps.json",FileAccess.WRITE); file.store_string(JSON.stringify(rows,"\t")); file.close()
