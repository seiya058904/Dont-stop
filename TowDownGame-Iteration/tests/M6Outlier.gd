...
func _ready():
	...
	await wait(0.3)
	Demo.save_path = "res://evidence/m6-outlier-"+tier+".json"
	...
	PlayerData.player_level = 10; PlayerData.player_hp_max += 4.5; PlayerData.player_hp = PlayerData.player_hp_max
	...
	for stage in [13]:
		...
		var file = FileAccess.open("res://docs/iteration/evidence/m7/regression-artifacts/outlier-close-"+tier+".json",FileAccess.WRITE)
...
func _process(delta):
	...
	if distance > 50 or not Combat.clear_line(Utils.player.global_position,target.global_position):
	...
	elif distance < 30: direction = target.global_position.direction_to(Utils.player.global_position).rotated(0.6)
	elif damage_clock > 1.5: direction = target.global_position.direction_to(Utils.player.global_position).orthogonal()
	if Engine.get_process_frames()%30 < 16:
		print("OUTLIER MOVE stage=",LevelServer.level," p=",Utils.player.global_position," target=",target.global_position," direction=",direction," hp=",PlayerData.player_hp," damage=",Combat.damage_events," gun=",Utils.player.gun.weapon_id," ammo=",Utils.player.gun.bullets_count," reload=",Utils.player.gun.is_reloading," aim=",background_aim," mouse=",Utils.player.gun.get_global_mouse_position()," tip=",Utils.player.gun.gun_tip.global_position," shotdir=",Utils.player.gun.direction)
