...
func _ready():
	...
	var lesson_buttons=Demo.ui.find_children("*","Button",true,false).filter(func(b): return b.text=="束缚教学")
	...
	while LevelServer.state=="COMBAT" and Time.get_ticks_msec()-lesson_end<7000: await wait(0.1)
	check(LevelServer.state=="CAMP" and get_tree().get_nodes_in_group("combat_transient").is_empty(),"root lesson automatically returns to camp and cleans attacks")
