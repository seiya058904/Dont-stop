...
func checkTouchState(id):
	var am :BaseAttachment = PlayerData.player_am_list[id]
	for node in weapon_am_nodes:
		node.setState(am.am_type == node.am_type and am.can_equip(choose_gun))

func checkTouchUp(id,node:WeaponAmItem):
	var am :BaseAttachment = PlayerData.player_am_list[id]
	if node.state and choose_gun.addAttachMent(am):
		loadWeaponAm()
...
func _unhandled_input(event: InputEvent) -> void:
	if Input.is_action_just_pressed("ui_cancel") and Demo.top_pause(self):
