...
out = root / 'docs/iteration/evidence/m9' / label
if out.exists():
    raise SystemExit('Evidence label already exists; use a new label')
run_root = allocate(support, 'm9', label)
source_root = root
baseline_revision = None
if '--baseline' in args:
    args.remove('--baseline')
    baseline_revision = 'b2f8f48'
...
if source_root != root:
    for fixture in (root / "tests").glob("M9*"):
...
for milestone in ['m12', 'm11', 'm8', 'm9']:
