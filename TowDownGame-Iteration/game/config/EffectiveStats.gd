...
static func calculate(gun, attachments: Array) -> Dictionary:
	var b = gun.base_stats
	var magazine_flat = 0
	var reload_mul = 1.0
	var damage_percent = PlayerData.base_bullet_damage + Demo.rank("T01") * 0.08
	...
	for am in attachments:
		match am.am_id:
			...
			122: jumps += 1
	return {
		"tags":gun.tags, "damage": (b.damage + PlayerData.player_damage) * (1.0 + damage_percent),
		"magazine": maxi(1, int((b.magazine + magazine_flat) * (1.0 + PlayerData.base_magazine_count + Demo.rank("T04") * 0.1))),
		"reload": maxf(0.15, b.reload * maxf(0.1, 1.0 - PlayerData.base_reload_speed - Demo.rank("T03") * 0.05) * reload_mul),
		"rate": clampf(b.rate * PlayerData.player_fire_rate * (1.0 + Demo.kill_stacks * Demo.rank("T10") * 0.03), 0.1, 60.0),
